...
def Res_sys_sim(I, e, s_0, s_min, s_max, env_min, d, Qreg):
    ...
    # Time length
    T = I.shape[0]
    # Minimum environmental compensation flow
    ...
    elif isinstance(Qreg['inflows'],(dict)):
    ...
    for k in range(T-1): # Loop for each time-step 
        ...
        # then the environmental compensation (env) = inflow (I). Otherwise env = d.
        if env_min[t] >= I[t] + Qreg_inf[t] :
            env[t] = I[t] + Qreg_inf[t]
        else:
            env[t] = env_min[t]
        # If the required environmental compensation is higher than the water resource available (s + I + Qreg_inf - e)
        # then the environmental compensation is equal to the higher value between 0 and the resource available  
        if env_min[t] >= s[t-1] - s_min + I[t] + Qreg_inf[t] - E[t]:
            env[t] = s[t-1] - s_min + I[t] + Qreg_inf[t] - E[t]
        else:
            env[t] = env_min[t]
        # If the demand (d) is higher than the water resource available (s + I + Qreg_inf - e - env)
        ...
        if d[t] >= s[t-1] - s_min + I[t] + Qreg_inf[t] - E[t] - env[t]:
            Qreg_rel[t] = np.min([Qreg_rel[t],np.max([0,s[t-1] - s_min + I[t] - E[t] + Qreg_inf[t] - env[t]])])
        # The spillage is equal to the higher value between 0 and the resource available exceeding the reservoir capacity
        spill[t] = np.max([0,s[t-1] + I[t] - Qreg_rel[t] - env[t] - E[t] + Qreg_inf[t] - s_max])
        # The final storage (initial storage in the next step) is equal to the storage + inflow - outflows
        s[t] = s[t-1] + I[t] + Qreg_inf[t] - Qreg_rel[t] - env[t] - E[t] - spill[t]
                
    outputs = (env, spill, Qreg_rel, Qreg_inf, s)
    return outputs